...
export default function Careers() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${process.env.SERVER_HOST}/career`);
              ...
              <tbody>
                {careers.map((career, index) => (
                  <tr key={index} className="hover:bg-gray-700 transition duration-150">
                    ...
                    <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                      <TableDropdown
                        editurl={`/admin/career/new/${career._id}`}
                        deleteurl={`${process.env.SERVER_HOST}/career/${career._id}`}
                        gridurl="/admin/manageCareers"
                      />
                    </td>
                  </tr>
                ))}
