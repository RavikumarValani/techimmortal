...
export default function CareerStatus({ isOpen, setIsOpen, data, careers,filterData, careerError }) {
    ...
    const submitStatus = async () => {
        if (formData.description) {
            const response = await axios.put(`${process.env.SERVER_HOST}/career/job/${data.id}`, formData, {
                headers: {
            ...
            if (response.data.success) {
                toggleModal();
                const careerData = await axios.get(`${process.env.SERVER_HOST}/career/job/all`);
                careers(careerData.data.careers);
                filterData(careerData.data.careers);
                careerError(response.data.message)
            } else {
                toggleModal();
                careerError(response.data.message)
            }
