...
export default function AddCareer() {
  ...
  const submitPost = async (e) => {
    ...
    if (validate.success) {
      ...
      const response = await axios.put(`${process.env.SERVER_HOST}/career/${id}`, formDataToSend, {
        headers: {
  ...
  useEffect(() => {
    if (id) {
      const fetchData = async () => {
        try {
          const response = await axios.get(`${process.env.SERVER_HOST}/career/${id}`);
          setResponsibility(JSON.parse(response.data.career.responsibility));
